...
const App = () => {
 ...
 const [assetTypeInstructions, setAssetTypeInstructions] = useState(null); // NEW: For instructions
 ...
 // FIX 1: Handle country selection and filter brands WITH SORTING
 const handleCountryChange = (countryCode) => {
 setSelectedCountry(countryCode);
 setSelectedBrands(new Set());
 setAssetTypeInstructions(null); // Clear instructions on selection change
 ...
 // FIX 1: Sort alphabetically by brand name
 .sort((a, b) => {
 ...
 setAssetTypeInstructions(null); // Clear instructions on brand selection change
 };
 ...
 setCopyError(null);
 setAssetTypeInstructions(null); // Clear previous instructions
 ...
 // FIX 2: Helper function to determine if entity should be shown
 const shouldShowEntity = (brand) => {
 if (!brand.entity) return false;
 
 // Special case: Don't show "Benriach" for "The Glendronach"
 if (brand.name.toLowerCase().includes('glendronach') && brand.entity.toLowerCase().includes('benriach')) {
 return false;
 }
 ...
 {/* FIX 2: Only show entity when it adds useful context, formatted as "by Entity" */}
 ...
 {/* Asset Type Instructions Card - NEW */}
